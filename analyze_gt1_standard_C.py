...
def analyze_gt1_distribution():
    ...
    # Define score criteria (Standard C)
    # below2: 0~6
    # below1: 7~11
    # on: 12~15
    # above1: 16~17
    ...
    print("Segment Counts for GT1 (Standard C):")
    print(segment_counts)
    
    # Calculate percentages
    segment_percentages = (segment_counts / len(df_filtered) * 100).round(1)
    print("\nSegment Percentages for GT1 (Standard C):")
    print(segment_percentages)

    # Visualization
    fig = plt.figure(figsize=(12, 10))
    plt.suptitle(f'2024년 5월 {target_level} 학생 성적 분석 (Standard C)', fontsize=16, y=0.95)
    ...
    # Fix FutureWarning by assigning hue
    sns.barplot(x=segment_counts.index, y=segment_counts.values, hue=segment_counts.index, palette=[color]*5, ax=ax2, legend=False)
    ...
    plt.savefig('gt1_distribution_analysis_C.png', dpi=300)
    print("\nAnalysis complete. Chart saved as 'gt1_distribution_analysis_C.png'")
